...
require "copytable"
require "colors"
--dofile (GetInfo(60) .. "custom_colors.lua") --might need to readd this ...
...
function TextRect:addColorLine(line)
   self:addStyles(line) --can I just remove colourstostyles here? ColoursToStyles(line)
end

function TextRect:addStyles(styles)
   -- extract URLs so we can add our movespots later
   local urls = self:findURLs(styles) --can I remove strip? strip_colours_from_styles(styles)
...
function TextRect:copyPlain()
   self:copyAndNotify(self:selected_text()) --can we remove strip colours? strip_colours(self:selected_text())
end

function TextRect:copy()
   self:copyAndNotify(self:selected_text()) --can we remove cononicalize? canonicalize_colours(self:selected_text(), true)
end
...
function TextRect:selected_text()
   ...
   function store_message()
      if current_message[1] then
         ...
         table.insert(s_text, current_message) --can I just remove stylestocolours here? StylesToColours(current_message)
         current_message = {}
      end
   end
